...
const NODE_W = 130
const NODE_H = 168
const GEN_GAP = 220   // вертикальный отступ между поколениями
const NODE_GAP = 50   // горизонтальный отступ между узлами
const PAD = 60        // отступ вокруг дерева
...
function buildLayout(persons: Person[], rels: Rel[]): NodeData[] {
  ...
  // parent-child карта (parent → [children])
  ...
  for (const r of rels) {
    if (r.relation_type === 'parent') {
      ...
      ;(childrenOf[r.person2_id] ??= []).push(r.person1_id)
      ;(parentsOf[r.person1_id] ??= []).push(r.person2_id)
    }
  }

  // BFS: раздаём поколения
  const gen: Record<string, number> = {}
  const ids = persons.map(p => p.id)
  const roots = ids.filter(id => !parentsOf[id]?.length)
  ...
  // Максимальная ширина — чтобы центрировать узкие поколения
  const genList = Object.entries(byGen).map(([g, list]) => ({ g: +g, list }))
...
export default function TreePage() {
  ...
  // SVG размер
  const svgW = nodes.length ? Math.max(...nodes.map(n => n.x + NODE_W)) + PAD : 600
  const svgH = nodes.length ? Math.max(...nodes.map(n => n.y + NODE_H)) + PAD : 400

  // Рёбра
  type Edge = { path: string; dashed: boolean }
  const edges: Edge[] = []
  for (const r of rels) {
    ...
    if (r.relation_type === 'parent') {
      edges.push({
        path: bezierPath(a.x + NODE_W / 2, a.y + NODE_H, b.x + NODE_W / 2, b.y),
        dashed: false,
      })
    } else if (r.relation_type === 'child') {
      edges.push({
        path: bezierPath(b.x + NODE_W / 2, b.y + NODE_H, a.x + NODE_W / 2, a.y),
        dashed: false,
      })
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col">
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-auto">
            ...
            {nodes.map(node => {
              ...
              return (
                ...
                >
                  ...
                  {/* Кнопка-подсказка */}
                  <rect
                    x={x + 12} y={y + NODE_H - 28}
                    width={NODE_W - 24} height={18}
                    rx={6}
                    fill="#f5f5f4"
                  />
                  <text
                    x={x + NODE_W / 2} y={y + NODE_H - 15}
                    textAnchor="middle"
                    fontSize={9}
                    fill="#a8a29e"
                    fontFamily="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif"
          ...
          {/* Легенда */}
          <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-white border border-stone-200 rounded-xl px-4 py-2.5 flex items-center gap-5 text-xs text-stone-400 shadow-sm">
            ...
            <span className="text-stone-300">Нажмите на карточку чтобы открыть профиль</span>
          </div>
        </div>
      )}
    </div>
